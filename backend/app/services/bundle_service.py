...
def _resolve_goal(goal: str) -> dict:
    ...
    return {"category": None, "title": "Custom Bundle", "tags": [], "query": goal}


def _select_within_budget(
    candidates: list[Product], budget: float | None, max_items: int = 5
) -> list[Product]:
    """Greedy selection favouring variety: pick complementary items (cheapest
    first) until the budget is spent or we hit max_items."""
    chosen: list[Product] = []
    total = 0.0
...
def build_bundle(db: Session, goal: str, budget: float | None = None) -> dict:
    ...
    if len(candidates) < 2 and cfg.get("category"):
        candidates = search_products(db, category=cfg["category"], limit=50)

    chosen = _select_within_budget(candidates, budget)

    if len(chosen) < 2:
        raise BundleNotPossible(
            "I couldn't build a bundle that fits within that budget. Try increasing "
            "the budget or changing the goal."
        )
    ...
    explanation = (
        f"I picked {len(chosen)} complementary products ({names}) that work well "
        f"together for your goal"
        + (f" and stay within ₹{int(budget)}." if budget else ".")
    )
...
def make_cheaper(db: Session, goal: str, current_total: float) -> dict:
    """Rebuild the bundle with a lower budget target (~85% of current)."""
    target = round(current_total * 0.85, 2)
    return build_bundle(db, goal, budget=target)
